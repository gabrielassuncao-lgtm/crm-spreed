...
function LeadsTab({ funnels, cards }) {
  ...
  const origins = [...new Set(cards.map(c => c.origin).filter(Boolean))].sort((a, b) => a.localeCompare(b));
...
/* ---------- RELATORIOS TAB ---------- */
function RelatoriosTab({ funnels, cards: allCards }) {
  const { theme } = useTheme();
  const [filterResponsible, setFilterResponsible] = useState('all');
  const [filterOrigin, setFilterOrigin] = useState('all');

  const origins = [...new Set(allCards.map(c => c.origin).filter(Boolean))].sort((a, b) => a.localeCompare(b));
