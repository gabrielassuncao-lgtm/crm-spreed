    ...
    <div style={{ minHeight: 420, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 320 }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: '#6366F1', margin: '0 auto 12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <GitBranch size={22} color="#fff" />
          </div>
          <h1 style={{ fontSize: 18, fontWeight: 700, margin: 0, color: '#F8FAFC' }}>CRM de vendas</h1>
        ...
        <div style={{ position: 'relative', marginBottom: 10 }}>
          <div style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', color: '#64748B' }}><UserIcon size={14} /></div>
          <input placeholder="E-mail" value={email} onChange={e => setEmail(e.target.value)} style={inputStyle} />
        </div>
        <div style={{ position: 'relative', marginBottom: 10 }}>
          <div style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', color: '#64748B' }}><Lock size={14} /></div>
        ...
        {err && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#FCA5A5', fontSize: 12.5, marginBottom: 10 }}>
            <AlertCircle size={13} /> {err}
          </div>
        )}
        {info && <div style={{ color: '#93C5FD', fontSize: 12.5, marginBottom: 10 }}>{info}</div>}

        <button onClick={submit} disabled={loading} style={{ ...primaryBtn, width: '100%', marginTop: 4, opacity: loading ? 0.6 : 1 }}>
